    ...
    SubShader 
	{
		...
		Pass 
		{
			...
			v2f vert (a2v v) 
			{
				...
				//避免内凹模型可能发生正面遮挡的问题。
				//书上的写法
				//把顶点发现的z分量进行处理（设定为一个定值），归一化后再进行扩张
				//这样的结果会让背面更扁平化。
				viewNormal.z = -0.5;
				...
				float3 offset_pos_cs = viewPos.xyz + viewDir * _MaxOutlineZOffset;

				float linewidth = -viewPos.z / unity_CameraProjection[1].y;
				linewidth = sqrt(linewidth);

				//核心： viewPos = viewPos + viewNormal * _Outline;
				viewPos.xy = offset_pos_cs.xy + viewNormal.xy * linewidth * burn.y * _Outline * 1.1 * _OutsideNoiseWidth ;
				viewPos.z = offset_pos_cs.z;
